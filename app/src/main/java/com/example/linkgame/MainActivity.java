    ...
    private void playBGSound(String music) {
        ...
        switch (music) {
            ...
            case "InfinityWar":
                ...
                break;
            case "None":
                mp =MediaPlayer.create(MainActivity.this, R.raw.none);;
                break;
        }
        if (!isMusicStop) {
            mp.start();// 开始播放
...
//                    linearlayout.setBackgroud(getResources().getColor(R.color.bg1));
    ...
    private void share() {
        ...
        textIntent.putExtra(Intent.EXTRA_TEXT, "感谢分享！");
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        switch (bgm) {
            ...
            case 5:
                ...
                break;
            case 6:
                menu.findItem(R.id.shutdown).setChecked(true);
                break;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.song5:
                ...
                break;
            case R.id.shutdown:
                getSharedPreferences("linkgame", MODE_PRIVATE).edit().putInt("bgm", 6).apply();
                thread = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        playBGSound("None");// 播放背景音乐
                    }
                });
                thread.start();// 开启线程
                break;
